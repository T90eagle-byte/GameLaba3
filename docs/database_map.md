...
В описании процедур ЛР2 также нужен `linkage_group` для сцепленных генов. Перед DDL нужно согласовать добавление этого поля, потому что механика `get_linked_allele_set` и `crossbreed` без него не работает.
...
- `trait_description` - описание фенотипа.

ЛР2 в запросах и описании `get_phenotype` также использует `species_type` и отдельные фенотипические столбцы вроде цвета и размера. Это нужно уточнить перед DDL.
...
- Подтвердить физические имена таблиц: русские идентификаторы из PDF или snake_case имена без кавычек.
...
- Добавить или согласовать `genes.linkage_group`.
- Добавить или согласовать `creatures.species_type` и фенотипические столбцы.
