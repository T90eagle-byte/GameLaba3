...
## Content/GUI pass: формулировки заданий (обновлено)
- В seed обновлены только тексты tasks.description для устранения двусмысленности UX.
- Простые задачи формулируются как find/tutorial (Найдите / Отберите / предъявите).
- Backend не изменялся: check_task проверяет признаки существа, а не его происхождение.
