...
`schema.ready=true` означает, что обязательные таблицы, package `PKG_GENETICS_GAME`, его `VALID` spec/body, отсутствие package errors, миграционные инварианты, reference/seed-данные и web API signatures присутствуют. Для seed также проверяются mutation rules, task markers и покрытие всех видов. Набор пользователей ЛР3 не входит в это условие: его отдельно проверяет `database/scripts/check_lr3_demo_data.py`.
