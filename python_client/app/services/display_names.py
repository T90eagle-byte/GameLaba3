...
GENE_TYPE_LABELS = {
    ...
    "performance": "Функциональный признак",
}
...
MUTAGEN_TYPE_LABELS = {
    "RADIATION": "Радиационный мутаген",
...
TASK_NAME_LABELS = {
    ...
    "task_predator_fish_line": "Хищная линия костных рыб",
    "task_armored_crustacean": "Бронированный ракообразный",
    "task_dense_fur_mammal": "Млекопитающее с густой шерстью",
    "task_cartilaginous_fin_line": "Линия хрящевых с широким плавником",
...
def gene_label(value: Any, *, with_code: bool = False) -> str:
    ...
    key = code.lower()
    label = GENE_LABELS.get(key, code.replace("_", " "))
    if with_code and code != "Не указано":
        return f"{label} ({code})"
    return label


def gene_type_label(value: Any, *, with_code: bool = True) -> str:
    code = display_value(value)
    key = code.lower()
    label = GENE_TYPE_LABELS.get(key)
    if label is None:
        return code
    if with_code and code != "Не указано":
        return f"{label} ({code})"
...
def dominance_label(value: Any, *, with_code: bool = True) -> str:
    code = display_value(value).upper()
    label = DOMINANCE_LABELS.get(code)
    if label is None:
...
def experiment_type_label(value: Any, *, with_code: bool = True) -> str:
    code = display_value(value).upper()
    label = EXPERIMENT_TYPE_LABELS.get(code)
    if label is None:
        return display_value(value)
    if with_code and code != "НЕ УКАЗАНО":
        return f"{label} ({code})"
...
def mutagen_type_label(value: Any, *, with_code: bool = True) -> str:
    code = display_value(value).upper()
    label = MUTAGEN_TYPE_LABELS.get(code)
    if label is None:
        return display_value(value)
    if with_code and code != "НЕ УКАЗАНО":
        return f"{label} ({code})"
...
def mutation_type_label(value: Any) -> str:
    try:
        ...
        return display_value(value)
    label = MUTATION_TYPE_LABELS.get(mutation_type)
    if label is None:
...
def mutation_name_label(value: Any, *, with_code: bool = False) -> str:
    code = display_value(value)
    key = code.lower()
    label = MUTATION_NAME_LABELS.get(key, code.replace("_", " "))
    if with_code and code != "Не указано":
        return f"{label} ({code})"
    return label
...
def task_name_label(value: Any, *, with_code: bool = False) -> str:
    code = display_value(value)
    key = code.lower()
    label = TASK_NAME_LABELS.get(key, code.replace("_", " "))
    if with_code and code != "Не указано":
        return f"{label} ({code})"
    return label
...
def phenotype_summary_label(value: Any) -> str:
    ...
    for part in parts:
        if "=" not in part:
            lines.append(_trait_expression_label(part))
            continue

        gene_code, trait_code = part.split("=", 1)
        gene_text = gene_label(gene_code.strip())
        trait_text = _trait_expression_label(trait_code.strip())
        lines.append(f"{gene_text}: {trait_text}")
...
def _trait_expression_label(value: str) -> str:
    ...
    if lower_text.startswith("intermediate(") and text.endswith(")"):
        ...
        if left is not None and right is not None:
            return f"промежуточный ({trait_label(left)} / {trait_label(right)})"
        return f"промежуточный ({inside})"

    left, right = _split_pair(text)
    if left is not None and right is not None:
        return f"{trait_label(left)} / {trait_label(right)}"
